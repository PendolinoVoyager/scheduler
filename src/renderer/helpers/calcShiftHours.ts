...
export function calcShiftHours(cellData: CellData): number {
...
export function calcRestTime(cells: CellData[]) {
  if (!cells[1].startTime && !cells[0].endTime) return 48;
  if (!cells[1].startTime) return 48 - cells[0].endTime!;
  if (!cells[0].startTime && cells[1].startTime) return 24 + cells[1].startTime;
