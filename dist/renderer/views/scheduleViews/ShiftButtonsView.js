...
export default class ShiftButtonsView extends View {
    ...
    <div class="flex-row shift-form">
      ${Object.entries(CONFIG.SHIFT_TYPES)
            ...
            if (shift === 'Custom')
                return;
            return `<button type="submit" class="box-sharp ${shift.toLowerCase()}" data-shift="${shift}">${info.translation}</button>`;
        })
            .join('')}
       </div>
       ...
       <input type="time" name="start"></input>-
            ...
            <button type="submit" class="box-sharp custom" data-shift="Custom">${CONFIG.SHIFT_TYPES['Custom'].translation}</button>
       </div>
    `;
    }
}
